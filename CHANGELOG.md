...
- Development test mode is now enabled via Jupiter config: server `development.testMode` controls vanilly server management, permission, and world rule allowances; client `development.testMode` controls F3, world creation, brightness, LAN commands, and dev hints. It no longer reads the JVM's `infx.testMode` parameter, nor requires client and server switches to match.
