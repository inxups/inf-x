...
- **材质分级盾牌系列（替换工具格挡）**：新增 `EquipmentType.SHIELD` + `InfxShieldItem`（`FactoryKind.SHIELD`），为 wood/copper/silver/gold/iron/ancient_metal/mithril/adamantium 八种材质各注册一面盾。盾的 `BlocksAttacks` 按材质分级：减伤比例（木 60%→艾德曼 100%）、举起延迟（0.30s→0.10s）、破盾冷却倍率（1.25→0.45）、格挡损耗系数（1.0→0.30），`BYPASSES_SHIELD`（fall/fire/lava/cactus 等）穿透所有盾；耐久走 `material.toolDurability(2)`（木 400→艾德曼 204800）。原版 `minecraft:shield`/`shield_decoration` 配方保持禁用，盾改由对应材质工作台合成（木盾手工台、铜/银/金盾铜台、铁盾铁台、远古金属盾远古金属台、秘银盾秘银台、艾德曼盾艾德曼台）；盾仅可附耐久/经验修补类附魔（对齐原版盾，不误入 `MELEE_WEAPON_ENCHANTABLE`）；视觉复用原版盾特殊模型/贴图（per-material 贴图待所有者授权素材）。移除 `ItemProperties.toolBlocking()` 与工具上的 `BLOCKS_ATTACKS` 组件——工具不再格挡，盾成为唯一格挡途径（斧的 `disablesBlockingSeconds` 保留以 disable 对方盾）。GameTest `shields` 校验各材质减伤比例、配方加载与工具不再格挡，并做破坏断言验证（故意改错木盾期望值 0.60→0.61 确认测试会失败）。
### 问题修复
