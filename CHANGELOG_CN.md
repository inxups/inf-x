...
- **材质分级盾牌系列（替换工具格挡）**：新增 `EquipmentType.SHIELD` + `InfxShieldItem`（`FactoryKind.SHIELD`），精简为三档材质盾——wood / ancient_metal / adamantium。盾的 `BlocksAttacks` 按材质分级：减伤比例（木 60%→远古金属 85%→艾德曼 100%）、举起延迟（0.30s→0.10s）、破盾冷却倍率（1.25→0.45）、格挡损耗系数（1.0→0.30），`BYPASSES_SHIELD`（fall/fire/lava/cactus 等）穿透所有盾；耐久走 `material.toolDurability(2)`（木 400 / 远古金属 12800 / 艾德曼 204800）。原版 `minecraft:shield`/`shield_decoration` 配方保持禁用，盾改由对应工作台合成（木盾手工台、远古金属盾远古金属台、艾德曼盾艾德曼台）；盾仅可附耐久/经验修补类附魔（对齐原版盾，不误入 `MELEE_WEAPON_ENCHANTABLE`）。移除 `ItemProperties.toolBlocking()` 与工具上的 `BLOCKS_ATTACKS` 组件——工具不再格挡，盾成为唯一格挡途径（斧的 `disablesBlockingSeconds` 保留以 disable 对方盾）。
  - **per-material 贴图**：远古金属/艾德曼盾用各自 `infx:entity/shield/<material>_shield_base`(+`_nopattern`) 贴图，经自定义 `InfXShieldSpecialRenderer`（仿 vanilla `ShieldSpecialRenderer` + InfX `SafeSpecialRenderer` 范式，per-material `Sheets.SHIELD_MAPPER.apply` 取贴图；经 `RegisterSpecialModelRendererEvent` 注册为 `infx:shield`）；木盾复用原版 `minecraft:shield` 贴图（原版盾本即木/铁主题）。原 owner 提供的贴图文件名曾被误写（如 `copper_shield_base..png`），已 rename 至干净命名。盾贴图经 vanilla `shield_patterns.json` 的 `directory` sprite source 跨命名空间聚合自动注册（与 InfX 僵尸/苦力怕贴图同机制）。
  - GameTest `shields` 校验三档减伤比例、配方加载与工具不再格挡，并做破坏断言验证（故意改错木盾期望值 0.60→0.61 确认测试会失败）。
### 问题修复
- **迅捷潜行误进附魔台**：`minecraft:swift_sneak` 在 vanilla 26.1 属 `#treasure`（`in_enchanting_table = #non_treasure`，本不进附魔台），但 `InfXEnchantments.TREASURE_ONLY` 漏列，使其混入 `TABLE`、可在附魔台 roll 到护腿。补入 `TREASURE_ONLY`（现含两诅咒/soul_speed/frost_walker/swift_sneak），`in_enchanting_table` 标签经 runData 同步剔除；仍属 `ALL`，故战利品/交易/怪物装备仍可产出，符合原版宝藏语义。
- **月相对齐 MITE 三处偏差**：① 血月雷暴窗口由 noon→1AM（raw `[6_000, 19_000)`）修正为 MITE 的 6AM→7PM（raw `[0, 13_000)`），与 `World.java:8675-8680` 的 `WeatherEvent(first_tick_of_day + 6_000, 13_000)` 对齐——`first_tick_of_day + 6_000` 落在未调整日始 = raw 0 = 黎明（非 noon），原 infx 注释"from noon"误读了 `first_tick_of_day`（实为午夜）；② 月晕（MoonDog/PHANTOM）月亮亮度由误与蓝月同组 1.1 改回 MITE 默认公式（`factor × 0.5 + 0.75`），MITE `getMoonBrightness` switch 不含 MoonDog；③ 月晕夜不再出幻影波（MITE 月晕纯视觉，刷怪不引用 `isMoonDog`），仅血月夜出幻影波（`SpawnGate.phantomWaveCount` 去掉 `PHANTOM` 分支，`MonsterEvents` 消费方随之生效）。同步更新 `BasicSystemsTest` 与 `infx_phantom_moon_spawns` GameTest 断言。
