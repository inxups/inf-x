...
## 0t7
### 项目方向
- 放弃与 MITE 的对齐：今后以 26.1.2 / NeoForge 原版行为为基准开发，现有行为改动全部保留；清理代码注释、测试文案与标识符中的 MITE 字样（材质来源清单、资源路径、NBT 键与原版 Smite 附魔等溯源/功能性引用保持不变）。
...
- 各种材质的 INFX 胡萝卜钓竿与诡异菌钓竿现在可以正常控制并加速坐骑：原版 26.1 的猪/炽足兽只认精确的原版 `carrot_on_a_stick`/`warped_fungus_on_a_stick` 为控制物，导致手持 INFX 钓竿骑乘时既不能转向、加速也无效；现已放宽为所有 INFX 材质钓竿（含原版猪与 INFX 猪、原版炽足兽）。
- INFX 胡萝卜钓竿/诡异菌钓竿手持模型改为原版钓竿姿态（`handheld_rod`），不再以普通物品姿态平举。
...
## 0t6
### 问题修复
- INFX 剑、镰刀、斧等所有目录武器不再携带 ATTACK_RANGE 组件，近战攻击距离遵循与空手/原版武器相同的 1.5 格规则（创造模式 5 格），客户端拾取回归原版路径：骑乘时准星不再选中自己的坐骑，攻击不会再误伤坐骑。
