...
public final class MiteSkeleton extends Skeleton implements MiteMob {
    ...
    private @Nullable MiteHardLimitedBowAttackGoal<MiteSkeleton> bowGoal;

    public MiteSkeleton(EntityType<? extends Skeleton> type, Level level) {
        super(type, level);
        ...
        switch (variant()) {
            case SKELETON -> {
                if (random.nextFloat() < 0.25F) {
                    setItemSlot(EquipmentSlot.MAINHAND, equipment(MiteMaterial.WOOD, EquipmentType.CLUB));
                }
            }
            ...
            case BONE_LORD -> equip(MiteMaterial.RUSTED_IRON, true, lordWeapon(level.getLevel()));
            case ANCIENT_BONE_LORD -> equip(MiteMaterial.ANCIENT_METAL, true, lordWeapon(level.getLevel()));
        }
    ...
    private void equip(MiteMaterial material, boolean plateArmor, EquipmentType weapon) {
    ...
    public void performRangedAttack(LivingEntity target, float power) {
        ...
        ItemStack ammunition = equipment(
                variant() == Variant.LONGDEAD ? MiteMaterial.ANCIENT_METAL : MiteMaterial.RUSTED_IRON,
                EquipmentType.ARROW);
    ...
    public void aiStep() {
        ...
        for (AbstractSkeleton skeleton : level.getEntitiesOfClass(
            ...
            skeleton.heal(1.0F);
            if (skeleton instanceof MiteSkeleton troop) {
                troop.inspire();
            }
