...
public final class ReconnectData extends SavedData {
    ...
    private static final Codec<ReconnectData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                    RESTRICTIONS.optionalFieldOf("restrictions", Map.of()).forGetter(data -> data.restrictions))
            .apply(instance, ReconnectData::new));
    public static final SavedDataType<ReconnectData> TYPE = new SavedDataType<>(
            InfiniteX.id("r196_reconnect_limits"), ReconnectData::new, CODEC);
    ...
    private ReconnectData(Map<String, Restriction> restrictions) {
        this.restrictions = new HashMap<>(restrictions);
    }

    public static ReconnectData get(MinecraftServer server) {
