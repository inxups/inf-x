...
public final class R196CurseData extends SavedData {
    private static final Codec<UUID> UUID_CODEC = Codec.STRING.xmap(UUID::fromString, UUID::toString);
    private static final Codec<Map<String, Entry>> ENTRIES = Codec.unboundedMap(Codec.STRING, Entry.CODEC);
    static final Codec<R196CurseData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                    ENTRIES.optionalFieldOf("entries", Map.of()).forGetter(data -> data.entries))
            .apply(instance, R196CurseData::new));

    public static final SavedDataType<R196CurseData> TYPE = new SavedDataType<>(
            InfiniteX.id("r196_witch_curses"), R196CurseData::new, CODEC);

    private final Map<String, Entry> entries;

    public R196CurseData() {
        this(Map.of());
    }

    private R196CurseData(Map<String, Entry> entries) {
        this.entries = new HashMap<>(entries);
    }

    public static R196CurseData get(MinecraftServer server) {
    ...
    public boolean add(UUID player, UUID witch, R196CurseType type, long realizationTick) {
    ...
    public record Entry(
        ...
        static final Codec<Entry> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                        ...
                        R196CurseType.CODEC.fieldOf("type").forGetter(Entry::type),
