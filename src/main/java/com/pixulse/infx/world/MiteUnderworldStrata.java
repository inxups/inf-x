...
public final class MiteUnderworldStrata {
    ...
    static StrataPlan plan(long worldSeed, ChunkPos chunkPos) {
        MiteLegacyStrataNoise noiseSet =
                NOISES_BY_WORLD_SEED.computeIfAbsent(worldSeed, MiteLegacyStrataNoise::new);
    ...
    static final class StrataPlan {
        ...
        boolean hasBedrockAt(int localX, int localZ, int relativeY) {
            return relativeY >= 0
                    && relativeY < LEGACY_TERRAIN_START_Y
                    && this.replacementAt(localX, localZ, relativeY) == BEDROCK;
        }
    ...
    private static final class MiteLegacyStrataNoise {
        ...
        private MiteLegacyStrataNoise(long worldSeed) {
            Random random = new Random(worldSeed);
