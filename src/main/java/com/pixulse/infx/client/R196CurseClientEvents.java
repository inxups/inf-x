...
public final class R196CurseClientEvents {
    private static final int MAX_EDGE = 28;

    private R196CurseClientEvents() {}
    ...
    private static void tick(ClientTickEvent.Post event) {
        ...
        if (player != null
                && player.isSprinting()
                && R196CurseManager.hasCurse(player, R196CurseType.CANNOT_RUN)) {
    ...
    private static void renderVignette(RenderGuiEvent.Pre event) {
        ...
        if (minecraft.player == null || !R196CurseManager.status(minecraft.player).active()) return;
    ...
    private static void renderStatus(RenderGuiEvent.Post event) {
        ...
        R196CurseStatus status = R196CurseManager.status(minecraft.player);
