 ...
 */
public final class InfxCraftingRules {
    ...
    private InfxCraftingRules() {}

    /**
     * Returns the INFX component value for one occupied crafting slot.
     *
    ...
    public static float componentDifficulty(ItemStack stack) {
        ...
        // Values explicitly present in the INFX source.  Keep these before
    ...
    /** INFX's per-level crafting reduction: +2% per displayed level (negative debt slows craft). */
    public static double levelModifier(int level) {
        int capped = Math.clamp(level, -Experience.MAX_DEBT_LEVEL, Experience.MAX_DISPLAY_LEVEL);
        return capped * 0.02D;
    }
    ...
    private static BenchTier gridTier(CraftingRecipe recipe) {
        ...
        return recipe.display().stream()
                        .map(InfxCraftingRules::displayGridTier)
                        .max(InfxCraftingRules::compareTier)
