...
/** Server menu for INFX's emerald (50 power) and diamond (100 power) tables. */
public final class InfxEnchantmentMenu extends EnchantmentMenu {
    ...
    public InfxEnchantmentMenu(int containerId, Inventory inventory, ContainerLevelAccess access, Kind kind) {
        ...
        accessors().infx$access().execute((level, pos) -> {
            ...
            if (isConversionInput(stack)) {
                for (int index = 0; index < 3; index++) {
                    ...
                    costs[index] = EnchantmentRules.experienceCost(CONVERSION_POWER);
                    enchantClue[index] = -1;
                    levelClue[index] = -1;
                }
                broadcastChanges();
            ...
            int tablePower = EnchantmentRules.enchantingTablePower(
                    bookshelves, kind.powerPerShelf(), kind.maximumPower());
            int effectivePower = EnchantmentRules.effectiveEnchantmentPower(
            ...
            for (int index = 0; index < 3; index++) {
                ...
                enchantmentPowers[index] = EnchantmentRules.enchantmentOptionPower(
                ...
                costs[index] = EnchantmentRules.experienceCost(enchantmentPowers[index]);
            }
            for (int index = 0; index < 3; index++) {
    ...
    private static int itemEnchantability(ItemStack stack) {
        if (stack.is(Items.BOOK)) return EnchantmentRules.BOOK_ENCHANTABILITY;
    ...
    private List<EnchantmentInstance> enchantmentChoices(RegistryAccess registryAccess, ItemStack stack, int option) {
        ...
        return EnchantmentSelector.select(
    ...
    public boolean clickMenuButton(@NonNull Player player, int buttonId) {
        if (buttonId < 0 || buttonId >= costs.length) {
            Util.logAndPauseIfInIde(player.getPlainTextName() + " pressed invalid INFX enchantment button " + buttonId);
            return false;
        }
