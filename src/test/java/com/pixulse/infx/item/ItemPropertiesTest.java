...
class ItemPropertiesTest {
    ...
    void toolAttributesDisplayTheFinalR196Damage() {
        var flintHatchet =
                ItemProperties.toolAttributes(key(MiteMaterial.FLINT, EquipmentType.HATCHET));
        var adamantiumHammer = ItemProperties.toolAttributes(
                key(MiteMaterial.ADAMANTIUM, EquipmentType.WAR_HAMMER));
    ...
    void fractionalArmorSurvivesComponentConstruction() {
        var helmet =
                ItemProperties.armorAttributes(key(MiteMaterial.COPPER, EquipmentType.HELMET));
        var leggings = ItemProperties.armorAttributes(
                key(MiteMaterial.COPPER, EquipmentType.CHAINMAIL_LEGGINGS));
        assertEquals(
                35.0 / 24.0,
    ...
    void horseArmorUsesBodyAttributes() {
        var armor = ItemProperties.armorAttributes(
                key(MiteMaterial.ADAMANTIUM, EquipmentType.HORSE_ARMOR));
