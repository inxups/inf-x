...
class SwimRulesTest {
    ...
    void currentImpulseIsConstantRegardlessOfTouchedBlockCount() {
        Vec3 single = SwimRules.currentImpulse(new Vec3(1.0D, 0.0D, 0.0D), SwimRules.CURRENT_SCALE);
        Vec3 many = SwimRules.currentImpulse(new Vec3(6.0D, 0.0D, 0.0D), SwimRules.CURRENT_SCALE);
        assertEquals(SwimRules.CURRENT_SCALE, single.length(), 1.0E-9D);
        assertEquals(SwimRules.CURRENT_SCALE, many.length(), 1.0E-9D);
        assertEquals(single, many);
    }

    @Test
    void opposingFlowsCancelAndStillWaterPushesNothing() {
        assertEquals(Vec3.ZERO, SwimRules.currentImpulse(Vec3.ZERO, SwimRules.CURRENT_SCALE));
        assertEquals(
                Vec3.ZERO,
                SwimRules.currentImpulse(new Vec3(1.0D, 0.0D, 0.0D).add(-1.0D, 0.0D, 0.0D), SwimRules.CURRENT_SCALE));
    }
    ...
    void submergedSwimmerGetsFullSwimUpImpulse() {
        float factor = SwimRules.swimUpFactor(true, false, false, true, false, 0.0F);
        assertEquals(1.0F, factor);
        assertEquals(0.04D, SwimRules.swimUpImpulse(factor), 1.0E-9D);
    }

    @Test
    void surfaceAndFallingColumnBothCutSwimUpToSevenSixteenths() {
        assertEquals(
                SwimRules.SURFACE_FACTOR,
                SwimRules.swimUpFactor(false, false, false, true, false, 0.0F));
        assertEquals(
                SwimRules.SURFACE_FACTOR,
                SwimRules.swimUpFactor(true, true, false, true, false, 0.0F));
    }
    ...
    void drawnBowWhileSuspendedAppliesTheSurfacePenaltyOnTop() {
        assertEquals(
                SwimRules.SURFACE_FACTOR,
                SwimRules.swimUpFactor(true, false, true, true, false, 0.0F),
                1.0E-6F);
        assertEquals(
                SwimRules.SURFACE_FACTOR * SwimRules.SURFACE_FACTOR,
                SwimRules.swimUpFactor(false, false, true, true, false, 0.0F),
                1.0E-6F);
        assertEquals(1.0F, SwimRules.swimUpFactor(true, false, true, false, false, 0.0F));
    }

    @Test
    void slowdownCapsSwimUpAndWallContactPartlyRestoresIt() {
        assertEquals(0.6F, SwimRules.swimUpFactor(true, false, false, true, false, -0.4F), 1.0E-6F);
        assertEquals(
                0.6F * 0.7F + 0.3F,
                SwimRules.swimUpFactor(true, false, false, true, true, -0.4F),
                1.0E-6F);
        assertEquals(0.0F, SwimRules.swimUpFactor(true, false, false, true, false, -1.5F));
    }
    ...
    void wallContactDoesNotRestoreSwimUpAtTheSurface() {
        assertEquals(
                SwimRules.SURFACE_FACTOR,
                SwimRules.swimUpFactor(false, false, false, true, true, 0.0F));
    }
    ...
    void speedModifierFollowsAmplifierWeights() {
        ...
        assertEquals(0.2F, SwimRules.speedModifier(-1, 0, false), 1.0E-6F);
        assertEquals(0.0F, SwimRules.speedModifier(0, 0, false), 1.0E-6F);
        assertEquals(-0.75F, SwimRules.speedModifier(-1, -1, true), 1.0E-6F);
    }

    @Test
    void upstreamSwimmingStillMakesHeadway() {
        double still = terminalSpeed(SWIM_ACCELERATION);
        double upstream = terminalSpeed(SWIM_ACCELERATION - SwimRules.CURRENT_SCALE);
        double downstream = terminalSpeed(SWIM_ACCELERATION + SwimRules.CURRENT_SCALE);
    ...
    void sprintingWaterfallSwimmerStillDescendsAfterTheReducedSwimUpImpulse() {
        double reducedSwimUp = SwimRules.swimUpImpulse(SwimRules.SURFACE_FACTOR);
        double afterWaterDrag = reducedSwimUp * WATER_DRAG;
        double verticalMovement = SwimRules.sprintWaterfallVerticalMovement(
                afterWaterDrag, SwimRules.waterGravity(0.08D));
    ...
    void oppositionIsZeroWhenSwimmingWithOrAcrossTheCurrent() {
        ...
        assertEquals(0.0D, SwimRules.opposition(new Vec3(1.0D, 0.0D, 0.0D), current), 1.0E-9D);
        assertEquals(0.0D, SwimRules.opposition(new Vec3(0.0D, 0.0D, 1.0D), current), 1.0E-9D);
    }

    @Test
    void oppositionIsOneWhenSwimmingDirectlyUpstream() {
        Vec3 current = new Vec3(1.0D, 0.0D, 0.0D);
        assertEquals(1.0D, SwimRules.opposition(new Vec3(-1.0D, 0.0D, 0.0D), current), 1.0E-9D);
    }
    ...
    void oppositionIsPartialAtAnAngleAndIgnoresVerticalMotion() {
        ...
        assertEquals(1.0D, SwimRules.opposition(backAndUp, current), 1.0E-9D);
        Vec3 diagonal = new Vec3(-1.0D, 0.0D, 1.0D);
        assertEquals(Math.sqrt(2.0D) / 2.0D, SwimRules.opposition(diagonal, current), 1.0E-9D);
    }

    @Test
    void oppositionIsZeroWhenMovementOrCurrentIsNegligible() {
        Vec3 current = new Vec3(1.0D, 0.0D, 0.0D);
        assertEquals(0.0D, SwimRules.opposition(Vec3.ZERO, current), 1.0E-9D);
        assertEquals(0.0D, SwimRules.opposition(new Vec3(-1.0D, 0.0D, 0.0D), Vec3.ZERO), 1.0E-9D);
    }
    ...
    void antiCurrentSprintDragOnlyKicksInWhileSwimmingUpstream() {
        ...
        assertEquals(
                sprintSlowDown,
                SwimRules.antiCurrentSprintDrag(
        ...
        double partial = SwimRules.antiCurrentSprintDrag(
